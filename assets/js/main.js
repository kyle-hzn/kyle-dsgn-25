...
function handleScroll() {
  ...
  if (st > lastScroll && st > 50) {
    gsapWithCSS.to(header, { y: "-120%", duration: 0.3, ease: "power2.out" });
  } else {
    gsapWithCSS.to(header, { y: "0%", duration: 0.3, ease: "power2.out" });
  }
  lastScroll = st;
}
document.addEventListener("DOMContentLoaded", () => {
...
if (spinItems.length) {
  spinItems.forEach((item) => {
    const spinTween = gsapWithCSS.to(item, {
      rotation: 360,
      duration: 15,
      ease: "none",
      repeat: -1
    ...
    ScrollTrigger.create({
      ...
      onUpdate: (self) => {
        ...
        const factor = gsapWithCSS.utils.clamp(
          1,
          // baseline
          5,
          // max boost
      ...
      onLeave: () => {
        gsapWithCSS.to(spinTween, { timeScale: 1, duration: 0.6, ease: "power2.out" });
      },
      onLeaveBack: () => {
        gsapWithCSS.to(spinTween, { timeScale: 1, duration: 0.6, ease: "power2.out" });
      }
...
gsapWithCSS.registerPlugin(ScrollTrigger);
function splitIntoLines$1(el) {
  const words = el.innerHTML.trim().replace(/\s+/g, " ").split(" ");
  el.innerHTML = "";
  const wordSpans = words.filter((w) => w.length > 0).map((word) => {
...
document.addEventListener("DOMContentLoaded", () => {
  document.querySelectorAll(".reveal-text.immediate").forEach((el) => {
    const lines = splitIntoLines$1(el);
    gsapWithCSS.from(lines, {
      yPercent: 100,
      opacity: 0,
      duration: 0.8,
      ease: "power3.out",
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll("a[href]").forEach((link) => {
    ...
    if (!href || href.startsWith("#") || href.startsWith("http") || link.closest(".header") || link.closest(".mobile-drawer") || link.classList.contains("header__logo"))
      return;
